    ...
    try
        ...
        if (projectState == 2) %Project state == 1 means the AQuA WBSv Backend instance is completely new and uninitialized
            ...
            if (~fh.sbs.Value) %If single viewport mode
                ...
                frameBakeTemp = img.patchRasterizeBake(fh.ims.im1.CData, getappdata(f,'bd')); %This function does not flip for us
				if (fh.overlayDat.Value == "Events")
					movPatchGraphicalObjects = findobj(fh.mov,'Type','Patch');

					color = [1 0.8500 0];

					frameBakeTemp = flipud(frameBakeTemp);
					for i = size(movPatchGraphicalObjects,1):-1:1
						if (strcmp(movPatchGraphicalObjects(i).Tag,'flex'))
							verticesArray = movPatchGraphicalObjects(i).Vertices;
							color = movPatchGraphicalObjects(i).EdgeColor;
							for p = 1:size(verticesArray,1)
								frameBakeTemp(verticesArray(p,2),verticesArray(p,1),1) = color(1);
								frameBakeTemp(verticesArray(p,2),verticesArray(p,1),2) = color(2);
								frameBakeTemp(verticesArray(p,2),verticesArray(p,1),3) = color(3);
							end
						end
					end
				...
				movtextGraphicalObjects = findobj(fh.mov,'Type','text');
				frameBakeTempHeight = size(frameBakeTemp,1);
				for i = size(movtextGraphicalObjects,1):-1:1
						textPosition = movtextGraphicalObjects(i).Position;
						textPosition(2) = frameBakeTempHeight - textPosition(2);
						textMatrix = img.text2im(movtextGraphicalObjects(i).String);
						textMatrix = textMatrix(1:2:end, 1:2:end);
						for x = 1:(size(textMatrix,2))
							for y = 1:(size(textMatrix, 1))
								if (textMatrix(y,x) == 0)
									if ((textPosition(2) + y) > 0) && ((textPosition(2) + y) < size(frameBakeTemp,2)) && ((textPosition(1) + x) > 0) && ((textPosition(1) + x) < size(frameBakeTemp,1))
										frameBakeTemp(textPosition(2) + y,textPosition(1) + x,1) = 1;
										frameBakeTemp(textPosition(2) + y,textPosition(1) + x,2) = 1;
										frameBakeTemp(textPosition(2) + y,textPosition(1) + x,3) = 1;
									else
									end
								end
							end
                ...
                btSt = getappdata(f,'btSt');
                curTypeL = btSt.('leftView');
                if (curTypeL == "Raw + overlay" || curTypeL == "Maximum Projection" || curTypeL == "Average Projection")
                    frameBakeTemp = img.patchRasterizeBake(fh.ims.im2a.CData, getappdata(f,'bd'));
				
                    %If any following modes are selected, then bake the
                    %boundaries
					movPatchGraphicalObjects = findobj(fh.movL,'Type','Patch');
					movtextGraphicalObjects = findobj(fh.movL,'Type','text');
                ...
                curTypeR = btSt.('rightView');
                if (curTypeR == "Raw + overlay" || curTypeR == "Maximum Projection" || curTypeR == "Average Projection")
                    frameBakeTemp = img.patchRasterizeBake(fh.ims.im2b.CData, getappdata(f,'bd'));
                    %If any following modes are selected, then bake the
                    %boundaries
					movPatchGraphicalObjects = findobj(fh.movR,'Type','Patch');
					movtextGraphicalObjects = findobj(fh.movR,'Type','text');
                    ...
                    im2bB = writeBMP(imresize(im2uint8(frameBakeTemp), im2bDownSample));
                else
                    %Otherwise just feed forward the imaging data normally
			...
			existingFig = findobj('Type', 'figure', 'Name', 'WBSCurveRenderer');
			if isempty(existingFig) || ~ishandle(existingFig)
				existingFig = figure('Name', 'WBSCurveRenderer');
			else
				clf(existingFig);
			end
			existingFig.Position(3) = fh.curve.Position(3);
			existingFig.Position(4) = fh.curve.Position(4);
			uiAxes = uiaxes(existingFig, 'Position', [1 6 fh.curve.Position(3) fh.curve.Position(4)]);
			set(uiAxes, 'YTickLabel', []);
			copyobj(fh.curve.Children, existingFig.Children);
            [curveImage] = frame2im(getframe(existingFig));
            ...
            %Lock values of fh.sldBriL.Value to fh.sldBri1.
            ...
            responseFhValueArray = {["projStat"] [10000000] [projectState]... %{2}
                ...
                ["ovlyFeaE"] [10000000] [fh.overlayFeature.Enable]... %{266}
				...
				["GaussBSt"] [10000000] [btSt.GaussFilter]...	%{308}
				["curveXws"] [10000000] [fh.curve.Position(3)]... %{311}
				["curveYxw"] [10000000] [fh.curve.Position(4)]... %{314}
				["chaOptLi"] [10000000] [find(strcmp(fh.channelOptionL.Items,fh.channelOptionL.Value))]... %{317}
				["chaOptRi"] [10000000] [find(strcmp(fh.channelOptionR.Items,fh.channelOptionR.Value))]... %{320}
				["chaOptLe"] [10000000] [fh.channelOptionL.Enable]... %{323}
				["chaOptRe"] [10000000] [fh.channelOptionR.Enable]... %{326}
                ["TempSepr"] [10000000] [fh.whetherExtend.Value]... %{329}
                };
        end

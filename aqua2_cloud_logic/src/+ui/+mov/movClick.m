function movClick(~,~,f,op,lbl,cursorPointDataJSON)
...
cursorPointData = jsondecode(cursorPointDataJSON);
x = cursorPointData.corrected_x;
y = cursorPointData.corrected_y;

if isfield(cursorPointData, 'user_defined_name')
    answer = {cursorPointData.user_defined_name};
else
    answer = '';
end
...
x = round(axImageWidth*x);
y = 1 - y;
y = round(axImageHeight*y);

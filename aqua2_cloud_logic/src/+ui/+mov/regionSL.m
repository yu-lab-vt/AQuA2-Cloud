function regionSL(~,~,f,op,lbl,directory,suffix)
    bd = getappdata(f,'bd');
    ...
    if strcmp(op,'save')
        if strcmp(lbl,'cell')
            %definput = {'_Cell.mat'};
            %selname = inputdlg('Type desired suffix for Region file:','Region file',[1 75],definput);
        ...
        selpath = directory;
        path0 = join([selpath,filesep,selname],"");
	   ...
	   if(strcmp(ext,'.mat'))
		   loadContent = load(directory,'bd0');

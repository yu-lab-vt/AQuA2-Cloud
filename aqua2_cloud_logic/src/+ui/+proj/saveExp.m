...
if isempty(fts)
    Supervisor.instanceBusySQL(true, 'Please save after event detection');
    return
end
...
%% gather results
Supervisor.instanceBusySQL(true, 'Gathering results');
...
%% export
fh = guidata(f);
fout = [convertStringsToChars(path0),convertStringsToChars(filesep),convertStringsToChars(file0)];
...
if fh.expEvt.Value==1
    disp('Saving res file ...');
    Supervisor.instanceBusySQL(true, 'Saving res file');
...
if fh.expMov.Value==1
    disp('Writing movie ...');
    Supervisor.instanceBusySQL(true, 'Writing movie');
    if opts.sz(3)==1
...
if fh.expFt.Value==1
    Supervisor.instanceBusySQL(true, 'Writing feature table');

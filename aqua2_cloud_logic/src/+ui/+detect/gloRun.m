...
if(opts.detectGlo)
    ...
    %ff = waitbar(0,'Remove detected local events...');
    disp('Remove detected local events')
    Supervisor.instanceBusySQL(true, 'Remove detected local events');
    dF_glo1 = glo.removeDetected(dF1,evtLocalLst1);
    clear dF1;
    % active region
    disp('Global active region detection');
    Supervisor.instanceBusySQL(true, 'Global active region detection');
    [arLst1] = act.acDetect(dF_glo1,opts,evtSpatialMask,1);  % foreground and seed detection
    
    % temporal segmentation
    if(fh.needTemp.Value)
        disp('Temporal segmentation for global signal');
        Supervisor.instanceBusySQL(true, 'Temporal segmentation for global signal');
        datOrg1 = getappdata(f,'datOrg1');
        [seLst1,subEvtLst1,seLabel1,majorInfo1,opts,~,~,~] = se.seDetection(dF_glo1,datOrg1,arLst1,opts);
        clear datOrg1;
        if(fh.needSpa.Value)
            % spatial segmentation
            disp('Spatial segmentation for global signal');
            Supervisor.instanceBusySQL(true, 'Spatial segmentation for global signal');
            [riseLst1,datR1,evtLst1,~] = evt.se2evtTop(dF_glo1,seLst1,subEvtLst1,seLabel1,majorInfo1,opts);
        else
            evtLst1 = seLst1; riseLst1 = [];evtLst1 = seLst1; riseLst1 = []; datR1 = [];
        end
    else
    ...
    if(~opts.singleChannel)
        ...
        disp('Remove detected local events CH2');
        Supervisor.instanceBusySQL(true, 'Remove detected local events CH2');
        dF_glo2 = glo.removeDetected(dF2,evtLocalLst2);
        clear dF2;
        % active region
        disp('Global active region detection CH2');
        Supervisor.instanceBusySQL(true, 'Global active region detection CH2');
        [arLst2] = act.acDetect(dF_glo2,opts,evtSpatialMask,2);  % foreground and seed detection

        % temporal segmentation
        if(fh.needTemp.Value)
            disp('Temporal segmentation for global signal CH2');
            Supervisor.instanceBusySQL(true, 'Temporal segmentation for global signal CH2');
            datOrg2 = getappdata(f,'datOrg2');
            [seLst2,subEvtLst2,seLabel2,majorInfo2,opts,~,~,~] = se.seDetection(dF_glo2,datOrg2,arLst2,opts);
            clear datOrg2;
            if(fh.needSpa.Value)
                % spatial segmentation
                disp('Spatial segmentation for global signal CH2');
                Supervisor.instanceBusySQL(true, 'Spatial segmentation for global signal CH2');

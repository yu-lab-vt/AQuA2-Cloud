...
Supervisor.instanceBusySQL(true, 'Preprocessing');
...
if(isempty(preSetting) || ~isfield(opts,'alreadyProprecess') || ~opts.alreadyProprecess || ...
    ...
    % bleach correction
    disp('Bleach correction');
    Supervisor.instanceBusySQL(true, 'Bleach correction');
...
disp('Baseline modeling and noise modeling');
Supervisor.instanceBusySQL(true, 'Baseline modeling and noise modeling');
...
disp('Further baseline modeling and noise modeling');
Supervisor.instanceBusySQL(true, 'Further baseline modeling and noise modeling');
if(~opts.singleChannel)
    datOrg2 = getappdata(f,'datOrg2');
...
Supervisor.instanceBusySQL(true, 'Preprocessing Done');
setappdata(f,'opts',opts);

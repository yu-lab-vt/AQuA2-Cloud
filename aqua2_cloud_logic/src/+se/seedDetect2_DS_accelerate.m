function [Map,arLst] = seedDetect2_DS_accelerate(dF,datOrg,arLst,opts)
...
%                     hasPeak =  se.curveSignificance(curve,max(1,floor(min(it)/t_scl)),ceil(max(it)/t_scl),opts.sigThr);
%                     hasPeak =  se.curveSignificance2(curve,max(1,floor(min(it)/t_scl)),ceil(max(it)/t_scl),opts.sigThr);
